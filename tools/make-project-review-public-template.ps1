...
$baseDir = "C:\Users\yas\Documents\Codex\2026-05-21\xmind"
$outDir = Join-Path $baseDir "project-review-public-template-xmind"
...
function Topic {
  ...
  if ($Labels.Count -gt 0) {
    $labelXml = "<labels>" + (($Labels | ForEach-Object { "<label>$_</label>" }) -join "") + "</labels>"
  }

  $notesXml = ""
  if ($Notes) {
    $notesXml = "<notes><plain>$Notes</plain></notes>"
  }
  ...
  return "<topic id=""$Id"" modified-by=""Codex"" timestamp=""$timestamp""><title>$Title</title>$markerXml$labelXml$notesXml$childrenXml</topic>"
}
